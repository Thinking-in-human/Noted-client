...
### 2-1 문제: Clinent -> Server -> S3로 글꼴 요청
---
처음에는 서버에서 사용자가 지정한 글꼴에 대한 URL을 통해 AWS S3에 저장된 글꼴 파일(.woff2)을 받아와 포스트잇 글꼴에 적용하도록 하였습니다.
여기서 문제점이 사용자가 글꼴을 변경할때마다 계속 S3에서 글꼴을 불러오고 있어 글꼴이 로드되는 지연 및 깜빡임이 있었습니다.

<br>

### 2-2 시도: 글꼴 로드 시간을 줄이는 방법
---
1. HTML에서 `link` 요소 사용
...
⇒ 사용자 지정 글꼴을 정의하고 다양한 글꼴 스타일에 사용할 글꼴 파일을 지정하기 위해 @font-face 결정하였습니다.
